...
def get_taxonomy(taxid, conn, verbose=False):
    ...
    if not p:
        ...
        if newid and newid[0]:
            ...
            newid = cur.fetchone()
            if newid and newid[0]:
                sys.stderr.write(f"Node {taxid} has been deleted\n")
...
def all_ids_complete(conn, protein=False, verbose=False):
    ...
    sys.stderr.write(f"Collecting all the data. Please stand by.\n")
    sys.stderr.write(f"Warning, this will take a long time!!.\n")
    for p in cur.fetchall():
        t = TaxonNode(*p)
        data['node'][p[0]] = t
...
def all_ids(conn, verbose=False):
    ...
    cur = conn.cursor()
    sys.stderr.write(f"Collecting all the taxon data. Please stand by.\n")
    sys.stderr.write(f"Warning, this will take a long time!!.\n")
    s = time.time()
    exc = cur.execute("select tax_id from nodes")
    sys.stderr.write(f"Done in {time.time() - s} seconds!.\n")
    return exc.fetchall()
...
def taxonomy_hierarchy(tid, verbose=False):
    ...
    while tid != 1:
        if tid == 1:
            if verbose:
                sys.stderr.write(f"Taxonomy ID 1 found\n")
            return
        if not tid:
            if verbose:
                sys.stderr.write(f"No tid\n")
            return
        ...
        if tid not in data['node']:
            try:
                ...
                if verbose:
                    sys.stderr.write(f"{tid} is not in database. Can not continue\n")
                return

        if verbose:
            sys.stderr.write(f"tid: {tid} parent: {data['node'][tid].parent}\n")
    ...
    while t.parent != 1 and t.taxid != 1:
        if t.rank in wanted_levels:
            if n.scientific_name:
                taxlist[wanted_levels.index(t.rank)] = t.rank[0] + ":" + n.scientific_name
            elif n.common_name:
                taxlist[wanted_levels.index(t.rank)] = t.rank[0] + ":" + n.common_name
            else:
                taxlist[wanted_levels.index(t.rank)] = t.rank[0] + ":" + f"[no name] taxid: {tid}"
